...
export class CartPageComponent implements OnInit {
    ...
    loadCartData(): void {
        this.itemsInCart = this.customerService.getCustomerCart();
    }

    addItems(): void{

        const sampleItems = [
            {dishId: 1, name: 'Pasta al pomodoro', price: 10.50, quantity: 1},
            {dishId: 2, name: 'Pizza Margherita', price: 8.50, quantity: 1},
            {dishId:3, name: 'Sparkling Water', price: 1.50, quantity: 2}
        ];
        this.itemsInCart = sampleItems;
        this.customerService.saveCartToLocal(sampleItems);
    }
    ...
    changeQuantity(itemIndex: number, change: number): void{
        ...
        if(newQuantity < 1) {
            this.customerService.removeFromCart(item.dishId);
            this.itemsInCart.splice(itemIndex, 1);
        } else {

            item.quantity = newQuantity;
            this.customerService.saveCartToLocal(this.itemsInCart);
        }
        this.updateTotal();
    }

    removeItem(itemId: number): void {
        this.customerService.removeFromCart(itemId);
        this.itemsInCart = this.itemsInCart.filter(item => item.dishId !== itemId);
        this.updateTotal();
    }
    ...
    async confirmOrder(): Promise<void> {
        ...
        try{
            const result = await this.customerService.sendOrderToServer(
                1,
                this.itemsInCart,
